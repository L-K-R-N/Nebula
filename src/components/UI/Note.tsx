...
const NoteCloseBtn = styled.button<MarkIconProps>`
    ...
    opacity: ${(props) => props.isActive ? .8 : 0};
    transition: all 0.4s;
    border-radius: 999px;
    position: absolute;
    top: 0;
...
export const Note: React.FC<Props> = ({color, note, project}) => {
    ...
    return (
        ...
        >
            {note.value}
            <NoteCloseBtn onClick={(e) => handleNoteDelete(e)} isActive={isActive} title="Удалить заметку" disabled={!isActive}>
                <NoteIcon/>
            </NoteCloseBtn>
        </StyledNote>
    )
